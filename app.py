        ...
        for i, (_, row) in enumerate(results.iterrows()):
            ...
            with cols[i % 4]:
                with st.container(border=True):
                    # --- תיקון הספירה - עדכון ישיר של ה-state לפני הצגת הסיידבר ---
                    is_currently_selected = unique_item_id in st.session_state.selected_items
                    if st.checkbox("➕ בחר", value=is_currently_selected, key=f"chk_{unique_item_id}"):
