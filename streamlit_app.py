...
def get_gdrive_service():
    try:
        # כאן תדביק את המפתח הארוך שלך (Base64)
...
with st.sidebar:
    ...
    # שינוי כאן: ה-MOQ מתחיל ב-0 כברירת מחדל
    max_moq = st.number_input("MOQ מקסימלי", value=0, min_value=0, max_value=100000, step=500)
    
    max_delivery = st.slider("זמן אספקה מקסימלי (ימים)", 5, 90, 90, 5)
    selected_materials = st.multiselect("חומר", ["Stainless Steel", "Plastic", "Bamboo", "Glass"])
...
if not df.empty and search_input:
    ...
    if not results.empty:
        ...
        # סינון MOQ: רק אם הזינו מספר גדול מ-0
        if max_moq > 0:
            ...
            with cols[i % 4]:
                with st.container(border=True):
                    ...
                    if img_id:
                        ...
                        if b64: st.markdown(f'<img src="data:image/jpeg;base64,{b64}" style="width:100%; border-radius:8px;">', unsafe_allow_html=True)
                    st.write(f"**{row['item_key']}**")
                    for d in row['display_list'][:4]:
                        if not contains_chinese(d): st.write(f"<small>{d}</small>", unsafe_allow_html=True)
