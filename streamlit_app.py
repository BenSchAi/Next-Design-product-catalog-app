...
def extract_moq(details_list):
    for detail in details_list:
        if 'MOQ' in str(detail).upper():
            nums = re.findall(r'\d+', str(detail).replace(',', ''))
...
def load_all_data():
    ...
    for item in results.get('files', []):
        try:
            ...
            while not done: _, done = downloader.next_chunk()
            fh.seek(0)
            df_excel = pd.read_excel(fh, header=None)
            for idx in range(len(df_excel)):
                ...
                if any(k in row_str.upper() for k in ['ITEM NO', 'ITEM REF', 'ITEM:', 'DESCRIPTION']):
                    details = [str(val) for val in df_excel.iloc[idx:idx+20, :].values.flatten() if pd.notna(val) and str(val).strip()]
                    item_key = details[0]
                    full_text_str = " ".join(details)
                    all_products.append({
                        'item_key': item_key,
                        'display_list': details[:15],
...
if not df.empty and search_input:
    ...
    if not results.empty:
        ...
        for i, (_, row) in enumerate(results.iterrows()):
            unique_id = f"{row['base_filename']}_{i}"
            with cols[i % 4]:
                with st.container(border=True):
                    ...
                    if img_id:
                        ...
                        if b64:
                            st.markdown(f'<img src="data:image/jpeg;base64,{b64}" style="width:100%; height:220px; object-fit:contain; border-radius:8px; margin-bottom:10px;">', unsafe_allow_html=True)
                    ...
                    for detail in row['display_list']:
                        d_up = str(detail).upper()
                        if not contains_chinese(str(detail)) and not any(x in d_up for x in ['ITEM NO', 'ITEM:', 'MOQ:', 'FOB COST', 'FOB PORT', 'WEB', 'HTTP', 'VALIDITY']):
