    ...
    for excel in excels:
        ...
        try:
            ...
            for idx, img in enumerate(found_images):
                new_img_name = f"{base_name}_{idx}.png"
                if new_img_name.lower() in existing_imgs:
                    print(f"  ✅ {new_img_name} כבר קיים בדרייב.")
                    continue

                with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp:
                    img.save(tmp.name, format="PNG")
                    # הורדתי את מגבלת ה-KB - כל תמונה תעלה
                    media = MediaFileUpload(tmp.name, resumable=True)
                    service.files().create(
                        body={'name': new_img_name, 'parents': [FOLDER_ID_IMAGES]},
                        media_body=media
                    ).execute()
